...
model.fit(X, y)
print("Modèle LinearRegression entraîné sur toutes les villes, tous polluants, toutes dates.")

# ------------------------------------------------------------------
# 4) DÉTERMINER LA DERNIÈRE DATE MESURÉE
# ------------------------------------------------------------------
...
def get_last_lags(sub_df, last_date):
    ...
    et la mesure précédente. Si une mesure pour last_date existe, on l'utilise ;
    ...
    # Tenter de trouver une mesure exactement à last_date
    row_last = sub_df[sub_df["jour"] == last_date]
    if len(row_last) == 1:
        val_last = row_last.iloc[0]["valeur_journaliere"]
        # Récupérer la mesure juste avant last_date
        sub_before = sub_df[sub_df["jour"] < last_date]
        if len(sub_before) > 0:
            val_before = sub_before.iloc[-1]["valeur_journaliere"]
        else:
            val_before = val_last  # sinon, dupliquer la valeur
    else:
        # Si aucune mesure à last_date, prendre les deux dernières mesures disponibles
        if len(sub_df) >= 2:
...
while current_date < end_date:
    ...
    # S'assurer que toutes les colonnes nécessaires sont présentes et imputer les NaN
    ...
    # Mise à jour des prédictions et des lags pour chaque couple (ville, Polluant)
    for i, key in enumerate(keys):
        ...
        # Mise à jour des lags : lag_2 <- lag_1, lag_1 <- y_future
...
df_pred.to_csv("/Users/akkouh/Desktop/scd3/prediction_1year_all_cities_linear_regression.csv", sep=";", index=False,
               encoding="utf-8")

print("Fichier 'prediction_1year_all_cities_linear_regression.csv' créé.")
print("Aperçu :")
