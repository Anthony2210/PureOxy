...
/**
 * @file details.php
 * @brief Affiche les détails de la pollution atmosphérique pour une ville spécifique.
 *
 * Ce script gère la récupération et l'affichage des données de pollution pour une ville donnée.
...
if ($ville) {
     ...
     * @var string $sql Requête SQL pour récupérer les données de pollution
     */
    ...
    if ($result->num_rows === 0) {
        /**
         * Indique que la ville n'a pas été trouvée dans la base de données
         * @var bool $cityNotFound Indicateur de non-trouvabilité de la ville
         */
        $cityNotFound = true;
    } else {
        $cityNotFound = false;
         ...
         * @var string|int $population Population du département
         */
         ...
         * @var array $pollutants_data Données des polluants
         * @var array $dates Dates des mesures
         * @var array $arrondissements Arrondissements (si applicable)
         */
        $pollutants_data = [];
        $dates = [];
        $arrondissements = [];
        ...
        while ($row = $result->fetch_assoc()) {
             ...
             * @var array $mois_francais Tableau des mois
             */
            $mois_francais = [
                '01' => 'Janvier',
                '02' => 'Février',
             ...
             * @var string $formattedDate Date formatée
             */
             ...
             * @var string $columnIdentifier Identifiant unique pour la colonne
             */
         ...
         * @var array $city_pollution_averages Moyennes des polluants
         */
         ...
         * @var array $seuils Seuils réglementaires
         * @var array $seuil_types Types de normes
         */
         ...
         * @var array $seuil_types_filtered Types de normes filtrés
         */
        ...
        if (isset($_SESSION['user_id'])) {
             ...
             * @var bool $is_favorite Indicateur si la ville est favorite
             */
        ...
        /**
         * Ajouter ou retirer la ville des favoris en fonction de l'action POST
         */
         ...
         * @var bool $has_depassements Indicateur de dépassements
         */
    ...
    /**
     * Afficher une erreur si la ville n'est pas spécifiée
     */
...
<script>
    ...
    /**
     * Données détaillées des mesures et seuils
     */
     ...
     * @var Chart depassementsChart Instance du graphique des dépassements
     */
    var depassementsChart;

    /**
     * Fonction pour initialiser le graphique des dépassements
     * @param {string} selectedPolluant Symbole du polluant sélectionné
     * @param {Array} selectedSeuilTypes Types de seuil sélectionnés
     */
     ...
     * @param {string} typeNorme Type de norme
     * @return {string} Couleur en format RGBA
     */
    ...
    /**
     * Gestion des filtres pour le graphique des dépassements
     */
...
<script>
    ...
    /**
     * Gérer le hash dans l'URL pour le défilement
     */
    document.addEventListener("DOMContentLoaded", function () {
...
<script>
    /**
     * Gestion de l'ajout/removal des villes favorites via AJAX
     */
    document.addEventListener('DOMContentLoaded', function () {
